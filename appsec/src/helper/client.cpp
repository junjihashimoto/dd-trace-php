...
bool client::handle_command(const network::client_init::request &command)
{
    ...
    try {
        ...
        // save engine settings so we can recreate the service if rc path changes
...
bool client::handle_command(network::config_sync::request &command)
{
    ...
    // wait for new interface; see https://github.com/DataDog/dd-trace-php/pull/2735
...
bool client::handle_command(network::request_shutdown::request &command)
{
    ...
    update_remote_config_path(
        command.rem_cfg_path, response->meta, response->metrics);
