...
void runner::register_for_rc_notifications() {
    ...
    ddog_set_rc_notify_fn([](const ConfigInvariants *invariants,
                              const Arc_Target *target) {
        ...
        const std::shared_ptr<runner> runner = std::atomic_load(&RUNNER_FOR_NOTIFICATIONS);
        if (!runner) {
...
runner::~runner() {
    ...
    std::atomic_compare_exchange_strong(
        &RUNNER_FOR_NOTIFICATIONS, &expected, std::shared_ptr<runner>(nullptr));
}
...
void runner::resolve_symbols() {
    ...
    // NOLINTNEXTLINE
    ddog_remote_config_path = reinterpret_cast<decltype(ddog_remote_config_path)>(
